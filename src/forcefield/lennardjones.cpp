...
double LennardJones::comp_energy_mol(const std::vector<std::shared_ptr<Particle> > particles, Molecule* molecule)
{
    double energy = 0.;
...
*/
double LennardJones::comp_energy_par(std::vector<std::shared_ptr<Particle> > particles, const int i)
{
    std::valarray<double> force;
    return (comp_energy_par(particles, i, force, false));
}
...
double LennardJones::comp_energy_par(const std::vector<std::shared_ptr<Particle> > particles, const int i,
                                     std::valarray<double> &force, const bool comp_force)
{
    // declare variables
    int npar = particles.size();
